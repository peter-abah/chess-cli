...
module MoveGenerator
  ...
  PROMOTION_PIECES = [:Knight, :Bishop, :Rook, :Queen].freeze

  def moves_for_pos(pos)
  ...
  def moves_for_piece(piece)
    piece.move_sets.reduce([]) do |result, move_set|
      move_set.increments.each do |increment|
        result.concat gen_moves_for_increment(move_set, increment, piece)
      end

      result.concat gen_special_moves(move_set, piece)
    end
  end
  
  private
  
  def gen_moves_for_increment(move_set, increment, piece)
    x, y = increment.values_at(:x, :y)
    result = []

    1.upto(move_set.repeat) do |factor|
      new_pos = piece.position.increment(y: y * factor, x: x * factor)
      break if new_pos.out_of_bounds? || blocked?(move_set, piece, new_pos)
  ...
  def blocked?(move_set, piece, new_pos)
  ...
  def gen_special_moves(move_set, piece)
    result = []
    move_set.special_moves.each do |sym|
      result.concat self.send(SPECIAL_MOVES[sym], piece)
    end
    
    result.reject(&:nil?)
  end
  
  def castle_moves(piece)
    [kingside_castle_move(piece), queenside_castle_move(piece)].reject(&:nil?)
  end
  
  def en_passant_moves(piece)
    moves = [-1, 1].map do |x|
      from = piece.position
  ...
  def kingside_castle_move(piece)
    return unless board.can_castle_kingside? piece.color

    return unless kingside_castle_squares_available? piece
  ...
  def queenside_castle_move(piece)
    return unless board.can_castle_queenside? piece.color

    return unless queenside_castle_squares_available? piece
  ...
  def kingside_castle_squares_available?(piece)
  ...
  def queenside_castle_squares_available?(piece)
